...
const DEFAULT_AZURE_STORAGE_CONNECTION_STRING =
  ...
  "DefaultEndpointsProtocol=https;AccountName=webappsv2;AccountKey=N4UcrOzAJbGDOgPMgcA/c7J8rmsKMMMK98fEzDF8nijPCenndZNn4wTAAySln2lDK0MXyVRow+wC+AStduZu7A==;EndpointSuffix=core.windows.net";
const AZURE_IMAGE_CHUNK_SIZE = 32 * 1024;
...
function chunkString(value, chunkSize = AZURE_IMAGE_CHUNK_SIZE) {
  if (!value && value !== 0) {
    return [];
  }
...
function createAzureProfilesStore(options = {}) {
  ...
  async function listPartitionRowKeys(partitionKey) {
    await ensureReady();
    const filter = `PartitionKey eq '${escapeOdataValue(partitionKey)}'`;
    const entities = tableClient.listEntities({
      queryOptions: { filter },
  ...
  async function replaceProfile(profile) {
    ...
    const profileMeta = { ...rest, id: profile.id };
    const imageChunks = chunkString(image);
    const existingRowKeys = await listPartitionRowKeys(profile.id);
    const retainedRowKeys = new Set();

    await tableClient.upsertEntity({
      partitionKey: profile.id,
      rowKey: "meta",
      payload: JSON.stringify(profileMeta),
    });
    retainedRowKeys.add("meta");

    for (let idx = 0; idx < imageChunks.length; idx += 1) {
      const rowKey = `img-${String(idx + 1).padStart(6, "0")}`;
      await tableClient.upsertEntity({
        partitionKey: profile.id,
        rowKey,
        chunkData: imageChunks[idx],
      });
      retainedRowKeys.add(rowKey);
    }

    for (const rowKey of existingRowKeys) {
      if (!retainedRowKeys.has(rowKey)) {
        await tableClient.deleteEntity(profile.id, rowKey);
      }
    }
  }
  ...
  async function readProfiles() {
    ...
    for (const [partitionKey, bucket] of grouped.entries()) {
      ...
      if (
        (!profile.image || !profile.image.length) &&
